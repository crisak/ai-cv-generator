...
import type { LeadershipItem } from '@/types/experience'
...
const EMPTY_ITEM = (): LeadershipItem => ({
  id: uuidv4(),
  organization: '',
  location: '',
  role: '',
...
export function LeadershipSection({ items, onChange }: LeadershipSectionProps) {
  const [editItem, setEditItem] = useState<LeadershipItem | null>(null)
  ...
  function setField<K extends keyof LeadershipItem>(key: K, val: LeadershipItem[K]) {
    if (!editItem) return
      ...
      <Sheet open={!!editItem} onOpenChange={(v) => !v && setEditItem(null)}>
        <SheetContent className="sm:max-w-xl overflow-y-auto">
          <SheetHeader className="pb-4">
            <SheetTitle>{isNew ? 'Nuevo liderazgo' : 'Editar liderazgo'}</SheetTitle>
            <SheetDescription>
              {editItem?.organization
                ? `${editItem.organization} — ${editItem.role}`
                : 'Completa los datos'}
            </SheetDescription>
          </SheetHeader>
          ...
          {editItem && (
            <div className="space-y-4 pb-6">
              <div className="grid grid-cols-2 gap-3">
                ...
                <Field label="Rol de liderazgo">
                  <Input
                    value={editItem.role}
                    onChange={(e) => setField('role', e.target.value)}
                    placeholder="Tech Lead & Engineering Mentor"
                  />
                </Field>
              </div>
