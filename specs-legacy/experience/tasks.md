# Experience — Tasks

## Tareas completadas

- [x] Task 1: Crear schema RxDB para `experiences`
- [x] Task 2: Implementar `use-experience.ts` (CRUD)
- [x] Task 3: Formulario de datos básicos
- [x] Task 4: Sección de experiencia laboral con bullets editables
- [x] Task 5: Sección de educación
- [x] Task 6: Formulario de skills
- [x] Task 7: Sección de liderazgo
- [x] Task 8: Import/Export JSON
- [x] Task 9: Componente reutilizable `BulletList`

## Tareas pendientes (v2)

- [x] Task 10: Anonimizar datos personales en archivos de documentación
  - Scope: Reemplazar información personal real en `docs/cv-experiencia-real.json` y `docs/json-schema-cv-generator.json` con placeholders genéricos. Mantener estructura JSON, formato de bullets y coherencia de métricas. Nombres → "Nombre Ejemplo", empresas → "Empresa A/B/C", emails/teléfonos → placeholders.
  - Depende de: nada
