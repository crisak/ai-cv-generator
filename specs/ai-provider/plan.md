...
| Guard server-only | Sin directiva explícita | `'use server'` crea Server Actions (endpoints públicos), no es una guardia. `server-only` (paquete) es redundante porque Next.js App Router ya trata todo código sin `'use client'` como server-side por defecto |
